...
namespace NuvoControl.Server.FunctionServer
{
    ...
    class ConcreteSendMailCommand : ConcreteCommand
    {
        ...
        public override void execCommand(eCommandType cmdType, Function function)
        {
            ...
            {
                LogHelper.Log(LogLevel.Info, ">>> Execute SendMail command on event {0} ", cmdType);
                LogHelper.Log(LogLevel.Trace, "      SendMailCommand={0} / Function={1}", _sendMailCommand.ToString(), function.ToString());
                // Send mail ...
                ...
                if (!bOk)
                {
                    LogHelper.Log(LogLevel.Error, "      Execute SendMail FAILED!");
                }
            }
        }
