...
/// </summary>
Dictionary<int, IProtocol> _protocolDrivers = new Dictionary<int, IProtocol>();

/// <summary>
/// Private list holding all play sound drivers (for each audio device)
/// </summary>
Dictionary<int, IAudioDriver> _audioDrivers = new Dictionary<int, IAudioDriver>();

/// <summary>
/// Private list holding all OSC server drivers
/// </summary>
Dictionary<int, IOscDriver> _oscDrivers = new Dictionary<int, IOscDriver>();

/// <summary>
/// Holds a reference to the zone server.
/// </summary>
IZoneServer _zoneServer = null;

/// <summary>
/// Holds the reference to the osc server.
/// </summary>
NuvoControl.Server.OscServer.OscServer _oscServer = null;

/// <summary>
/// Holds a reference to the function server.
/// </summary>
NuvoControl.Server.FunctionServer.FunctionServer _functionServer = null;

/// <summary>
/// Private member to hold the timer used to periodically check for configuration changes
/// </summary>
System.Timers.Timer _timerCheckConfiguration = new System.Timers.Timer();
...
/// <param name="e"></param>
void _timerCheckConfiguration_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
{
    //Console.Write(".");
...
void _driver_onZoneStatusUpdate(object sender, ProtocolZoneUpdatedEventArgs e)
{
...
void InstantiateOscServer()
{
    ...
    _oscServer = new NuvoControl.Server.OscServer.OscServer(oscDeviceControllers);
    _oscServer.Start();
}
...
void AdjustDeviceSettings(Device device)
{
    if (_options != null)
    {
...
LogHelper.Log(LogLevel.Debug, String.Format("Check configuration timer started, each {0}[s]", NuvoControl.Server.WebConsole.Properties.Settings.Default.ConfigurationCheckIntervall));
_timerCheckConfiguration.Interval = (NuvoControl.Server.WebConsole.Properties.Settings.Default.ConfigurationCheckIntervall < 30 ? 30 : NuvoControl.Server.WebConsole.Properties.Settings.Default.ConfigurationCheckIntervall) * 1000;
